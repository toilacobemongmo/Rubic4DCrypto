/**
 * Chuẩn AES-128 (FIPS-197) thuần JavaScript hỗ trợ Uint8Array nhị phân
 */
const AES128 = (() => {
    const SBOX = [
        
    ];
    const INV_SBOX = new Uint8Array(256);
    SBOX.forEach((v, i) => INV_SBOX[v] = i);

    const RCON = [0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];

    function gmul(a, b) {
        let p = 0;
        for (let i = 0; i < 8; i++) {
            if (b & 1) p ^= a;
            const hi = a & 0x80;
            a = (a << 1) & 0xff;
            if (hi) a ^= 0x1b;
            b >>= 1;
        }
        return p;
    }

    function keyExpansion(keyBytes) {
        const w = new Uint8Array(176);
        for (let i = 0; i < 16; i++) w[i] = keyBytes[i];
        let bytesGenerated = 16;
        let rconIteration = 1;
        let temp = new Uint8Array(4);

        while (bytesGenerated < 176) {
            for (let i = 0; i < 4; i++) temp[i] = w[bytesGenerated - 4 + i];
            if (bytesGenerated % 16 === 0) {
                const t = temp[0];
                temp[0] = SBOX[temp[1]] ^ RCON[rconIteration++];
                temp[1] = SBOX[temp[2]];
                temp[2] = SBOX[temp[3]];
                temp[3] = SBOX[t];
            }
            for (let i = 0; i < 4; i++) {
                w[bytesGenerated] = w[bytesGenerated - 16] ^ temp[i];
                bytesGenerated++;
            }
        }
        return w;
    }

    function cipher(block, w) {
        let state = new Uint8Array(block);
        for (let i = 0; i < 16; i++) state[i] ^= w[i];

        for (let round = 1; round <= 10; round++) {
            for (let i = 0; i < 16; i++) state[i] = SBOX[state[i]];
            const s = new Uint8Array(state);
            state[1] = s[5];  state[5] = s[9];  state[9] = s[13]; state[13] = s[1];
            state[2] = s[10]; state[6] = s[14]; state[10] = s[2];  state[14] = s[6];
            state[3] = s[15]; state[7] = s[3];  state[11] = s[7];  state[15] = s[11];

            if (round < 10) {
                for (let c = 0; c < 4; c++) {
                    const idx = c * 4;
                    const a0 = state[idx], a1 = state[idx+1], a2 = state[idx+2], a3 = state[idx+3];
                    state[idx]   = gmul(a0, 2) ^ gmul(a1, 3) ^ a2 ^ a3;
                    state[idx+1] = a0 ^ gmul(a1, 2) ^ gmul(a2, 3) ^ a3;
                    state[idx+2] = a0 ^ a1 ^ gmul(a2, 2) ^ gmul(a3, 3);
                    state[idx+3] = gmul(a0, 3) ^ a1 ^ a2 ^ gmul(a3, 2);
                }
            }
            const offset = round * 16;
            for (let i = 0; i < 16; i++) state[i] ^= w[offset + i];
        }
        return state;
    }

    function invCipher(block, w) {
        let state = new Uint8Array(block);
        for (let i = 0; i < 16; i++) state[i] ^= w[160 + i];

        for (let round = 9; round >= 0; round--) {
            const s = new Uint8Array(state);
            state[1] = s[13]; state[5] = s[1];  state[9] = s[5];  state[13] = s[9];
            state[2] = s[10]; state[6] = s[14]; state[10] = s[2];  state[14] = s[6];
            state[3] = s[7];  state[7] = s[11]; state[11] = s[15]; state[15] = s[3];

            for (let i = 0; i < 16; i++) state[i] = INV_SBOX[state[i]];

            const offset = round * 16;
            for (let i = 0; i < 16; i++) state[i] ^= w[offset + i];

            if (round > 0) {
                for (let c = 0; c < 4; c++) {
                    const idx = c * 4;
                    const a0 = state[idx], a1 = state[idx+1], a2 = state[idx+2], a3 = state[idx+3];
                    state[idx]   = gmul(a0, 0x0e) ^ gmul(a1, 0x0b) ^ gmul(a2, 0x0d) ^ gmul(a3, 0x09);
                    state[idx+1] = gmul(a0, 0x09) ^ gmul(a1, 0x0e) ^ gmul(a2, 0x0b) ^ gmul(a3, 0x0d);
                    state[idx+2] = gmul(a0, 0x0d) ^ gmul(a1, 0x09) ^ gmul(a2, 0x0e) ^ gmul(a3, 0x0b);
                    state[idx+3] = gmul(a0, 0x0b) ^ gmul(a1, 0x0d) ^ gmul(a2, 0x09) ^ gmul(a3, 0x0e);
                }
            }
        }
        return state;
    }

    function formatKey(password) {
        const k = new Uint8Array(16);
        const enc = new TextEncoder().encode(password);
        for (let i = 0; i < enc.length; i++) k[i % 16] ^= enc[i];
        return k;
    }

    return {
        // Nhận vào Uint8Array, trả về Uint8Array
        encryptBytes: (bytes, password) => {
            const padLen = 16 - (bytes.length % 16);
            const padded = new Uint8Array(bytes.length + padLen);
            padded.set(bytes);
            padded.fill(padLen, bytes.length);

            const w = keyExpansion(formatKey(password));
            const out = new Uint8Array(padded.length);
            for (let i = 0; i < padded.length; i += 16) {
                out.set(cipher(padded.subarray(i, i + 16), w), i);
            }
            return out;
        },
        decryptBytes: (cipherBytes, password) => {
            if (cipherBytes.length === 0 || cipherBytes.length % 16 !== 0) {
                throw new Error("Dữ liệu cipher AES không hợp lệ (độ dài không chia hết cho 16)!");
            }
            const w = keyExpansion(formatKey(password));
            const out = new Uint8Array(cipherBytes.length);
            for (let i = 0; i < cipherBytes.length; i += 16) {
                out.set(invCipher(cipherBytes.subarray(i, i + 16), w), i);
            }
            const padLen = out[out.length - 1];
            if (padLen < 1 || padLen > 16) throw new Error("Khóa giải mã AES không chính xác!");
            return out.subarray(0, out.length - padLen);
        }
    };
})();